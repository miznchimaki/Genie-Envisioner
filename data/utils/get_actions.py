...
def normalize_angles(radius):  # normalize the radius angles into interval [-π, π]
...
def get_actions_eef(gripper, all_ends_p=None, all_ends_o=None, slices=None, delta_act_sidx=None):
    ...
    for i in slices:
        ...
        left_rpy = np.concatenate((all_ends_p[i, 0], rot_l.as_euler("xyz", degrees=False)), axis=0)  # left_rpy shape: (6, )
        rot_r = Rotation.from_quat(all_ends_o[i, 1])
        right_rpy = np.concatenate((all_ends_p[i, 1], rot_r.as_euler("xyz", degrees=False)), axis=0)  # right_rpy shape: (6, )
        all_left_rpy.append(left_rpy)
        all_right_rpy.append(right_rpy)

    ### xyz, rpy
    all_left_rpy = np.stack(all_left_rpy)    # all_left_rpy shape: (n, 6)
    all_right_rpy = np.stack(all_right_rpy)  # all_right_rpy shape: (n, 6)
