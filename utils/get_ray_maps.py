...
def get_ray_maps(intrinsic, c2w, H, W):
    ...
    fx, fy, cx, cy = intrinsic[:, 0, 0].unsqueeze(1).unsqueeze(2), \
        intrinsic[:, 1, 1].unsqueeze(1).unsqueeze(2), \
        intrinsic[:, 0, 2].unsqueeze(1).unsqueeze(2), \
        intrinsic[:, 1, 2].unsqueeze(1).unsqueeze(2)
    i, j = torch.meshgrid(
