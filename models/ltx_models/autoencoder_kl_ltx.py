...
class AutoencoderKLLTXVideo(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    @apply_forward_hook
    def encode(
        self, x: torch.Tensor, return_dict: bool = True, num_splits: int = 2,
        ...
        if self.use_slicing and x.shape[0] > 1:
            encoded_slices = [self._encode(x_slice) for x_slice in x.chunk(num_splits) if x_slice.numel() != 0]
    ...
    @apply_forward_hook
    def decode(
        self, z: torch.Tensor, temb: Optional[torch.Tensor] = None, return_dict: bool = True, num_splits: int = 2,
    ) -> Union[DecoderOutput, torch.Tensor]:
        """
